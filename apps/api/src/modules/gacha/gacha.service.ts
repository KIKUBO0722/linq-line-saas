...
export class GachaService {
  ...
  async createCampaign(tenantId: string, data: {
    ...
    try {
      ...
      this.logger.error(`Failed to create gacha campaign: ${error}`);
  ...
  async listCampaigns(tenantId: string) {
    try {
      ...
      this.logger.error(`Failed to list gacha campaigns: ${error}`);
      throw error instanceof HttpException ? error : new InternalServerErrorException('操作に失敗しました');
    }
  }
  ...
  async getCampaign(id: string, tenantId: string) {
    try {
      ...
      this.logger.error(`Failed to get gacha campaign ${id}: ${error}`);
      throw error instanceof HttpException ? error : new InternalServerErrorException('操作に失敗しました');
    }
  }
  ...
  async updateCampaign(id: string, tenantId: string, data: Partial<{
    ...
    try {
      ...
      this.logger.error(`Failed to update gacha campaign ${id}: ${error}`);
      throw error instanceof HttpException ? error : new InternalServerErrorException('操作に失敗しました');
    }
  }
  ...
  async deleteCampaign(id: string, tenantId: string) {
    try {
      ...
      this.logger.error(`Failed to delete gacha campaign ${id}: ${error}`);
      throw error instanceof HttpException ? error : new InternalServerErrorException('操作に失敗しました');
    }
  }
  ...
  async addPrize(campaignId: string, tenantId: string, data: {
    ...
    try {
      ...
      this.logger.error(`Failed to add prize to campaign ${campaignId}: ${error}`);
      throw error instanceof HttpException ? error : new InternalServerErrorException('操作に失敗しました');
    }
  }
  ...
  async draw(campaignId: string, tenantId: string, friendId?: string) {
    try {
      ...
      this.logger.error(`Failed to draw gacha for campaign ${campaignId}: ${error}`);
      throw error instanceof HttpException ? error : new InternalServerErrorException('操作に失敗しました');
    }
  }
  ...
  async getDrawHistory(campaignId: string, tenantId: string) {
    try {
      ...
      this.logger.error(`Failed to get draw history for campaign ${campaignId}: ${error}`);
      throw error instanceof HttpException ? error : new InternalServerErrorException('操作に失敗しました');
    }
  }
}
